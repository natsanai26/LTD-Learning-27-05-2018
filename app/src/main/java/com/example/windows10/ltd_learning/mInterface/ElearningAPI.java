...
public interface ElearningAPI {
    ...
    @GET("/elearning/course")
    Call<ResponseBody> getNewCourse(
            @Query("new") int numberOfNewCourse
    );

    @GET("/elearning/course")
    Call<ResponseBody> getTopCourse(
            @Query("top") int numberOfTopCourse
    );
    ...
    @GET("/elearning/dialogue")
    Call<ResponseBody> getCommentById(
      @Query("id") int id
    );
